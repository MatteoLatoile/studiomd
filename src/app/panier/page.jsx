...
function daysBetween(startISO, endISO) {
  ...
  const ms = b - a;
  const d = Math.floor(ms / 86400000) + 1; // inclusif
  return Math.max(1, d);
}
function fmtDate(d) {
  try {
...
export default function PanierPage() {
  ...
  return (
    <main className="bg-[#FFF5D7] min-h-screen py-30 px-4 md:px-8">
      ...
      <div className="mt-6 grid md:grid-cols-[1fr_360px] gap-6">
        {/* Liste items */}
        <section className="rounded-2xl bg-[#ffffffb3] ring-1 text-noir ring-black/5 shadow-[0_10px_30px_rgba(0,0,0,0.06)] p-4 sm:p-5">
          {items.length === 0 ? (
            <EmptyCart />
          ) : (
        ...
        <aside className="rounded-2xl bg-[#ffffffb3] ring-1 ring-black/5 shadow-[0_10px_30px_rgba(0,0,0,0.06)] p-4 sm:p-6 h-fit">
          ...
          <div className="mt-4 space-y-2 text-noir text-sm">
          ...
          <Link
            ...
            className="mt-5 inline-block w-fit rounded-xl px-5 py-3 font-semibold text-noir shadow disabled:opacity-60"
            style={{
              background: "linear-gradient(90deg,#FFC119 0%, #FFEB83 100%)",
            }}
          >
